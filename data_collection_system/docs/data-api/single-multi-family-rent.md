...
| 항목 | 내용 |
|---|---|
| API명 (영문) | Single-family/multi-family rental actual transaction price data |
| API명 (국문) | 단독/다가구 전월세 실거래가 자료 |
| API 설명 | 지역코드와 기간을 설정하여 해당지역, 해당기간의 단독다가구 전월세 실거래가 자료를 제공하는 단독다가구 전월세 실거래가 자료 조회 |
...
| 항목 | 내용 |
|---|---|
| 서비스 URL | `http://apis.data.go.kr/1613000/RTMSDataSvcSHRent` |
| 서비스 명세 URL (WSDL/WADL) | `http://apis.data.go.kr/1613000/RTMSDataSvcSHRent/getRTMSDataSvcSHRent?_wadl&type=xml` |
| 서비스 버전 | 1.0 |
| 서비스 시작일 | 2024.07.17 |
| 서비스 배포일 | 2024.07.17 |
...
| 번호 | API명 (영문) | 상세기능명 (국문) |
|---|---|---|
| 1 | getRTMSDataSvcSHRent | 단독/다가구 전월세 실거래가 자료 |
...
| 상세기능 번호 | 1 |
| 상세기능 유형 | 조회 (자료) |
| 상세기능명 (국문) | 단독/다가구 전월세 실거래가 자료 |
| 상세기능 설명 | 행정표준코드관리시스템(www.code.go.kr)의 법정동 코드 중 앞 5자리(예시: 서울 종로구 - 11110), 계약년월(예시: 201801)로 해당 지역, 해당 기간의 단독다가구 매매 신고정보를 조회 *(원문 그대로 — "전월세"가 아닌 "매매"로 표기되어 있음)* |
| Call Back URL | `https://apis.data.go.kr/1613000/RTMSDataSvcSHRent/getRTMSDataSvcSHRent` |
...
| totalCount | 전체 결과 수 | 전체 결과 수 | 4 | 1 | 166 |
| sggCd | 지역코드 | 지역코드 | 5 | 1 | 11110 |
| houseType | 주택유형 | 주택유형(단독/다가구) | 6 | 0 | 단독 |
| umdNm | 법정동 | 법정동 | 30 | 1 | 청운동 |
| totalFloorAr | 연면적 | 연면적 | 22 | 0 | 55 |
| dealYear | 계약년도 | 계약년도 | 4 | 1 | 2024 |
| dealMonth | 계약월 | 계약월 | 2 | 1 | 7 |
| dealDay | 계약일 | 계약일 | 2 | 1 | 26 |
| deposit | 보증금액 | 보증금액(만원) | 40 | 1 | 10,000 |
| monthlyRent | 월세금액 | 월세금액(만원) | 40 | 1 | 100 |
| buildYear | 건축년도 | 건축년도 | 4 | 0 | |
...
```
https://apis.data.go.kr/1613000/RTMSDataSvcSHRent/getRTMSDataSvcSHRent?serviceKey=서비스키&LAWD_CD=11110&DEAL_YMD=202407&pageNo=1&numOfRows=1
```
...
<response>
  ...
  <body>
    ...
    <pageNo>1</pageNo>
    <totalCount>166</totalCount>
  </body>
</response>
```

> ※ 원문 예제에서 `<contractType>` 안에 "단독"이 표기되어 있으나, 이는 원문 PDF 텍스트 추출 과정의 표기 오류로 보이며 실제로는 `<houseType>` 값입니다. 원문 그대로 옮겨두었으니 실제 응답 스펙 확인 시 참고하시기 바랍니다.
...
**단독다가구 전월세 자료 (구 API) → 단독다가구 전월세 실거래가 자료 (신규 API)**

| 구 컬럼명 | 구 항목명 | 신 컬럼명 | 신 항목명 |
|---|---|---|---|
| sggcd | 지역코드 | sggCd | 지역코드 |
| umdnm | 법정동 | umdNm | 법정동 |
| totalfloorarea | 계약면적 | totalFloorAr | 연면적 |

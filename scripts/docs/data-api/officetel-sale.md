...
| 항목 | 내용 |
|---|---|
| API명 (영문) | Officetel sales actual transaction price data |
| API명 (국문) | 오피스텔 매매 실거래가 자료 |
| API 설명 | 지역코드와 기간을 설정하여 해당지역, 해당기간의 오피스텔 매매 자료를 제공하는 오피스텔 매매 실거래가 상세 자료 조회 |
...
| 항목 | 내용 |
|---|---|
| 서비스 URL | `http://apis.data.go.kr/1613000/RTMSDataSvcOffiTrade` |
| 서비스 명세 URL (WSDL/WADL) | `http://apis.data.go.kr/1613000/RTMSDataSvcOffiTrade/getRTMSDataSvcOffiTrade?_wadl&type=xml` |
| 서비스 버전 | 1.0 |
| 서비스 시작일 | 2024.07.17 |
| 서비스 배포일 | 2024.07.17 |
...
| 번호 | API명 (영문) | 상세기능명 (국문) |
|---|---|---|
| 1 | getRTMSDataSvcAptRent | 오피스텔 매매 실거래가 자료 |

> ※ 원문에 상세기능명(영문)이 `getRTMSDataSvcAptRent`(아파트 전월세용 이름)로 기재되어 있으나, 서비스 URL·Call Back URL·요청 예제는 모두 `RTMSDataSvcOffiTrade`를 사용합니다. 지금까지 정리한 다른 API 문서들에서도 반복적으로 발견된 원문 자체의 복사-붙여넣기 오타로 보이며, 실제 호출 시에는 `getRTMSDataSvcOffiTrade`를 사용해야 합니다.
...
| 상세기능 번호 | 1 |
| 상세기능 유형 | 조회 (자료) |
| 상세기능명 (국문) | 오피스텔 매매 실거래가 자료 |
| 상세기능 설명 | 행정표준코드관리시스템(www.code.go.kr)의 법정동 코드 중 앞 5자리(예시: 서울 종로구 - 11110), 계약년월(예시: 201801)로 해당 지역, 해당 기간의 오피스텔 매매 실거래가 상세자료 조회 |
| Call Back URL | `http://apis.data.go.kr/1613000/RTMSDataSvcOffiTrade/getRTMSDataSvcOffiTrade` |
...
| totalCount | 전체 결과 수 | 전체 결과 수 | 4 | 1 | 27 |
| sggCd | 지역코드 | 지역코드 | 5 | 1 | 11110 |
| sggNm | 시군구 | 시군구 | 30 | 1 | 종로구 |
| umdNm | 법정동 | 법정동 | 60 | 1 | |
| jibun | 지번 | 지번 | 20 | 0 | 75 |
| offiNm | 단지명 | 단지명 | 100 | 1 | 용비어천가 |
| excluUseAr | 전용면적 | 전용면적 | 22 | 0 | 28.19 |
| dealYear | 계약년도 | 계약년도 | 4 | 1 | 2024 |
| dealMonth | 계약월 | 계약월 | 2 | 1 | 7 |
| dealDay | 계약일 | 계약일 | 2 | 1 | 17 |
| dealAmount | 거래금액 | 거래금액(만원) | 40 | 1 | 25,200 |
| floor | 층 | 층 | 10 | 0 | 9 |
| buildYear | 건축년도 | 건축년도 | 4 | 0 | 2004 |
...
> ※ 아파트/연립다세대 매매 API와 달리 오피스텔 매매 API는 `sggNm`(시군구명)이 응답에 별도로 포함됩니다.
...
```
https://apis.data.go.kr/1613000/RTMSDataSvcOffiTrade/getRTMSDataSvcOffiTrade?serviceKey=서비스키&LAWD_CD=11110&DEAL_YMD=202407&pageNo=1&numOfRows=1
```
...
**오피스텔 매매 신고 조회 서비스 (구 API) → 오피스텔 매매 실거래가 자료 (신규 API)**
